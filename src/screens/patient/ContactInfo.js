...
function ContactInfo({ navigation, route }) {
  const [selectedNav, setSelectedNav] = useState(""); // No active nav button
  const [isDeletePressed, setIsDeletePressed] = useState(false); // Track delete button press
  const [isChatPressed, setIsChatPressed] = useState(false); // Track chat button press
  const [isCallPressed, setIsCallPressed] = useState(false); // Track call button press
  const [isVideoPressed, setIsVideoPressed] = useState(false); // Track video button press
  ...
  // Ensure all contact fields are defined
  const safeContact = {
...
// Wrap ContactInfo with FontLoader
...
const styles = StyleSheet.create({
  ...
  actionButtonPressed: {
    backgroundColor: '#B1C4BD', // Same as normal state per spec
  },
  actionButtonText: {
    color: Colors.Accent,
    fontSize: 14,
